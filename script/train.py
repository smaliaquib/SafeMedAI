...
# ----------------------------- Argument Parser
def parse_args():
    parser = argparse.ArgumentParser(description="Train and register final classification model.")
    parser.add_argument("--config", type=str, required=False, default=os.getenv('MODEL_CONFIG'), help="Path to model_config.yaml")
    parser.add_argument("--data", type=str, required=False, default=os.getenv('DATASET_DIR'), help="Path to training Parquet file")
    parser.add_argument("--models-dir", type=str, required=False, default=os.getenv('MODEL_DIR'), help="Directory to save trained model")
...
def main(args):
    ...
    with mlflow.start_run(run_name="final_classification_training"):
        ...
        logger.info(f"Final F1: {f1:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}")
